...
**必须使用的高德工具**：
1. 步行路径规划（walking_direction）— 验证酒店到景点的步行距离
2. 公交路径规划（transit_direction）— 获取公交/地铁路线和时间
3. 驾车路径规划（driving_direction）— 获取驾车时间（考虑实时路况）
4. 骑行路径规划（bicycling_direction）— 短距离出行方案

要求：
1. 对同一天的景点做顺路排序（减少绕路）
2. 相邻景点之间做多交通方式（驾车/公交/步行）估时比选
3. 选出推荐交通方式（时间最优或最经济）
4. 考虑早晚高峰（7:00-9:00, 17:00-19:00）避免驾车
5. 酒店到第一个景点的步行距离要在合理范围内（<2km）
6. 生成替代路线
